...
getRegister' config plat expr =
  case expr of
    ...
    CmmReg reg ->
      return
        ( Fixed
            (cmmTypeFormat (cmmRegType plat reg))
    ...
    CmmRegOff reg off | isNbitEncodeable 12 (fromIntegral off) -> do
      ...
      where
        width = typeWidth (cmmRegType plat reg)
...
assignMem_IntCode rep addrE srcE =
  do
    ...
    return $
      COMMENT (text "CmmStore" <+> parens (text (show addrE)) <+> parens (text (show srcE)))
        `consOL` ( code
                    `appOL` addr_code
